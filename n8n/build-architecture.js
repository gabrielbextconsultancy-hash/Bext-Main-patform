...
// The report template, generated into the dashboard the same way the
// architecture map is. The email-preview API needs the deployed SELECT and the
// Render HTML node's code, and production runs the dashboard with no n8n/
// directory beside it — the filesystem read worked on the dev machine and
// returned ENOENT '/n8n/workflows/...' the first time the operator clicked the
// button in production. Bundling at build time is the pattern the repo already
// trusts, and this generator runs after every deploy, so the bundle moves in
// step with the workflows.
function generateReportTemplate() {
  const wf = JSON.parse(fs.readFileSync(
    path.join(ROOT, 'n8n/workflows/BEXT-Daily-News-5-Daily-Report.json'), 'utf8'));
  ...
  const out = path.join(ROOT, 'dashboard/src/lib/report-template.generated.ts');
  fs.writeFileSync(out, [
    '// GENERATED by n8n/build-architecture.js — do not edit.',
    '// The deployed Daily Report workflow: its window SELECT and its Render HTML',
    '// node code, bundled so the email-preview API renders the real artefact',
    '// without needing the n8n/ directory at runtime.',
    'export const REPORT_SELECT_SQL = ' + JSON.stringify(selectSql) + ';',
    'export const REPORT_RENDER_CODE = ' + JSON.stringify(renderCode) + ';',
    '',
  ].join('\n'), 'utf8');
